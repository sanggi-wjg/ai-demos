...
def openapi_agent_node(state: State):
    ...
    agent = planner.create_openapi_agent(
        ...
        requests_wrapper=RequestsWrapper(
            headers={"Authorization": f"Bearer {access_token_input}"} if access_token_input else {}
        ),
        llm=llm,
    ...
    chat_response = agent.invoke(state["user_input"])
    return {
        "messages": state["messages"] + [AIMessage(content=chat_response["output"], name="openapi_agent")],
    }
...
with open(f"oas_agent_using_langgraph.png", "wb") as f:
...
- If an API need a **UUID**, use random values.

### 4. Error Handling & Logging
- Log request and response details for each API call.
- If an API fails, analyze the failure reason and continue testing remains.
""".strip()

events = graph.stream({"user_input": prompt})
for event in events:
