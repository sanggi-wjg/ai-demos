...
def _is_pydantic_class(obj: Any) -> bool:
    return isinstance(obj, type) and (is_basemodel_subclass(obj) or BaseModel in obj.__bases__)
...
@deprecated(since="0.0.64", removal="1.0", alternative_import="langchain_ollama.ChatOllama")  # type: ignore[arg-type]
class OllamaFunctions(ChatOllama):
    ...
    def _generate(
        ...
        **kwargs: Any,
        ...
        response_message = super()._generate([system_message] + messages, stop=stop, run_manager=run_manager, **kwargs)
        ...
        if called_tool is None or called_tool["name"] == DEFAULT_RESPONSE_FUNCTION["name"]:
            if "tool_input" in parsed_chat_result and "response" in parsed_chat_result["tool_input"]:
                ...
                raise ValueError(f"Failed to parse a response from {self.model} output: " f"{chat_generation_content}")
            return ChatResult(
                generations=[
                    ChatGeneration(
                        message=AIMessage(
    ...
    async def _agenerate(
        ...
        **kwargs: Any,
        ...
        if called_tool is None:
            raise ValueError(f"Failed to parse a function call from {self.model} output: " f"{chat_generation_content}")
